  ...
  render() {
    ...
    return (
      <TableRow className={classes.row} key={this.props.id}>
        ...
        <TableCell>{this.props.patient}</TableCell>
        <TableCell numeric>{moment(this.props.date).format('DD MMM YYYY')}</TableCell>
        <TableCell numeric>{this.props.time}</TableCell>
        <TableCell>
          <Button size="small" onClick={() => { /*this.props.confirmAppointment(this.props.id);*/ }}>
            <i class="far fa-clipboard-check" /> confirme le rdv
          </Button>
        </TableCell>
      </TableRow>
    );
...
export default withStyles(styles)(PatientAppointmentsList);
